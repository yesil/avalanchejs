...
export abstract class PayloadBase {
  ...
  fromBuffer(bytes:Buffer, offset:number = 0):number {
    const size:number = bintools.copyFrom(bytes, offset, offset + 4).readUInt32BE(0);
    offset += 4;
...
export class BIGNUMPayload extends PayloadBase {
  ...
  constructor(payload:any = undefined) {
    ...
    if (payload instanceof Buffer) {
      ...
      const bufferPayload = bintools.fromBNToBuffer(payload);
      if (bufferPayload) {
        this.payload = bufferPayload;
      }
    } else if (typeof payload === 'string') {
      this.payload = Buffer.from(payload, 'hex');
    }
...
export class JSONPayload extends PayloadBase {
  ...
  constructor(payload:any = undefined) {
    ...
    if (payload instanceof Buffer) {
      ...
      const jsonstr:string = JSON.stringify(payload);
