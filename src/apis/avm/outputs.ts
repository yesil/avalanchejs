...
import {
  Output, StandardAmountOutput, StandardTransferableOutput, BaseNFTOutput,
} from '../../common/output';
...
export const SelectOutputClass = (outputid:number, ...args:Array<any>):Output => {
  ...
  } if (outputid === AVMConstants.SECPMINTOUTPUTID || outputid === AVMConstants.SECPMINTOUTPUTID_CODECONE) {
    return new SECPMintOutput(...args);
  } if (outputid === AVMConstants.NFTMINTOUTPUTID || outputid === AVMConstants.NFTMINTOUTPUTID_CODECONE) {
    return new NFTMintOutput(...args);
  } if (outputid === AVMConstants.NFTXFEROUTPUTID || outputid === AVMConstants.NFTXFEROUTPUTID_CODECONE) {
...
export class NFTMintOutput extends NFTOutput {
  ...
  toBuffer():Buffer {
    ...
    const bsize:number = this.groupID.length + superbuff.length;
    const barr:Array<Buffer> = [this.groupID, superbuff];
