...
import { SECPMintOutput, SECPTransferOutput } from './outputs';
import { InitialStates } from './initialstates';
import { UnixNow } from '../../utils/helperfunctions';
import { JRPCAPI } from '../../common/jrpcapi';
import { RequestResponseData } from '../../common/apibase';
import {
  Defaults, PrimaryAssetAlias, ONEAVAX,
...
export class AVMAPI extends JRPCAPI {
  ...
  getBlockchainAlias = (): string => {
    if (this.blockchainAlias === undefined) {
      const blockchain = this.getBlockchain(this.blockchainID);
      if (blockchain) {
        this.blockchainAlias = blockchain.alias;
  ...
  /**
   * Helper function to return a blockchain from its ID.
   * @param blockchainID ID of the blockchain to retrieve
   */
  getBlockchain(blockchainID?: string) {
    const netid: number = this.core.getNetworkID();
    const network = Defaults.network.get(netid);
    if (network) {
      const blockchain = blockchainID
        ? network.chains[blockchainID]
        : network.chains.X;
      return blockchain;
    }
    return undefined;
  ...
  refreshBlockchainID = (blockchainID?: string): boolean => {
    if (!blockchainID) {
      const blockchain = this.getBlockchain(blockchainID);
      if (blockchain) {
        this.blockchainID = blockchain.blockchainID;
        return true;
      }
      return false;
    }
   ...
   */
  getDefaultTxFee = (): BN => {
    const txFee = this.getBlockchain()?.txFee;
    if (txFee) {
      return typeof txFee === 'number' ? new BN(txFee) : txFee;
    }
    return new BN(0);
  };
  ...
  getDefaultCreationTxFee = (): BN => {
    const creationTxFee = this.getBlockchain()?.creationTxFee;
    if (creationTxFee) {
      return typeof creationTxFee === 'number'
        ? new BN(creationTxFee)
        : creationTxFee;
    }
    return new BN(0);
  };
  ...
  getBalance = async (address: string, assetID: string): Promise<object> => {
    ...
    return this.callMethod('avm.getBalance', params).then(
      (response: RequestResponseData) => response.data.result,
    );
  };
  ...
  createAddress = async (
    ...
    return this.callMethod('avm.createAddress', params).then(
      (response: RequestResponseData) => response.data.result.address,
    );
  };
  ...
  createFixedCapAsset = async (
    ...
    return this.callMethod('avm.createFixedCapAsset', params).then(
      (response: RequestResponseData) => response.data.result.assetID,
    );
  };
  ...
  createVariableCapAsset = async (
    ...
    return this.callMethod('avm.createVariableCapAsset', params).then(
      (response: RequestResponseData) => response.data.result.assetID,
    );
  };
  ...
  mint = async (
    ...
    return this.callMethod('avm.mint', params).then(
      (response: RequestResponseData) => response.data.result.txID,
    );
  };
  ...
  exportKey = async (
    ...
    return this.callMethod('avm.exportKey', params).then(
      (response: RequestResponseData) => response.data.result.privateKey,
    );
  };
  ...
  importKey = async (
    ...
    return this.callMethod('avm.importKey', params).then(
      (response: RequestResponseData) => response.data.result.address,
    );
  };
  ...
  import = async (
    ...
    return this.callMethod('avm.import', params).then(
      (response: RequestResponseData) => response.data.result.txID,
    );
  };
  ...
  importAVAX = async (
    ...
    return this.callMethod('avm.importAVAX', params).then(
      (response: RequestResponseData) => response.data.result.txID,
    );
  };
  ...
  listAddresses = async (
    ...
    return this.callMethod('avm.listAddresses', params).then(
      (response: RequestResponseData) => response.data.result.addresses,
    );
  };
  ...
  getAllBalances = async (address: string): Promise<Array<object>> => {
    ...
    return this.callMethod('avm.getAllBalances', params).then(
      (response: RequestResponseData) => response.data.result.balances,
    );
  };
  ...
  getAssetDescription = async (
    ...
    return this.callMethod('avm.getAssetDescription', params).then(
  ...
  getTx = async (txid: string): Promise<string> => {
    ...
    return this.callMethod('avm.getTx', params).then(
      (response: RequestResponseData) => response.data.result.tx,
    );
  };
  ...
  getTxStatus = async (txid: string): Promise<string> => {
    ...
    return this.callMethod('avm.getTxStatus', params).then(
      (response: RequestResponseData) => response.data.result.status,
    );
  };
  ...
  getUTXOs = async (
    ...
    return this.callMethod('avm.getUTXOs', params).then(
      (response: RequestResponseData) => {
  ...
  buildBaseTx = async (
    ...
    memo: PayloadBase | Buffer,
    asOf: BN = UnixNow(),
    locktime: BN = new BN(0),
    threshold: number = 1,
  ): Promise<UnsignedTx> => {
    const to: Array<Buffer> = this._cleanAddressArray(
      toAddresses,
      'buildBaseTx',
    ).map((a) => bintools.stringToAddress(a));
    const from: Array<Buffer> = this._cleanAddressArray(
      fromAddresses,
      'buildBaseTx',
    ).map((a) => bintools.stringToAddress(a));
    const change: Array<Buffer> = this._cleanAddressArray(
      changeAddresses,
      'buildBaseTx',
    ).map((a) => bintools.stringToAddress(a));
  ...
  buildImportTx = async (
    ...
    if (sourceChain === undefined) {
      ...
      throw new Error(
        `Error - AVMAPI.buildImportTx: Invalid destinationChain type: ${typeof sourceChain}`,
      );
    }

    if (srcChain === null) throw new Error('srcChain could not be deduced');

    const atomicUTXOs: UTXOSet = await (
      await this.getUTXOs(ownerAddresses, srcChain, 0)
    ).utxos;
    const avaxAssetID: Buffer = await this.getAVAXAssetID();
    ...
    if (atomics.length === 0) {
      throw new Error(
        `Error - AVMAPI.buildImportTx: No atomic UTXOs to import from ${srcChain} using addresses: ${ownerAddresses.join(
          ', ',
        )}`,
      );
    }
  ...
  buildExportTx = async (
    ...
    if (typeof destinationChain === 'undefined') {
      ...
      throw new Error(
        `Error - AVMAPI.buildExportTx: Invalid destinationChain type: ${typeof destinationChain}`,
      );
    }
  ...
  buildCreateAssetTx = async (
    ...
    symbol: string,
    denomination: number,
    mintOutputs: Array<SECPMintOutput>,
    memo: PayloadBase | Buffer,
    asOf: BN = UnixNow(),
  ): Promise<UnsignedTx> => {
    const from: Array<Buffer> = this._cleanAddressArray(
    ...
    if (symbol.length > AVMConstants.SYMBOLMAXLEN) {
      /* istanbul ignore next */
      throw new Error(
        `Error - AVMAPI.buildCreateAssetTx: Symbols may not exceed length of ${AVMConstants.SYMBOLMAXLEN}`,
      );
    }
    /* istanbul ignore next */
    if (name.length > AVMConstants.ASSETNAMELEN) {
      /* istanbul ignore next */
      throw new Error(
        `Error - AVMAPI.buildCreateAssetTx: Names may not exceed length of ${AVMConstants.ASSETNAMELEN}`,
      );
    }
  ...
  buildCreateNFTAssetTx = async (
    ...
    if (name.length > AVMConstants.ASSETNAMELEN) {
      /* istanbul ignore next */
      throw new Error(
        `Error - AVMAPI.buildCreateNFTAssetTx: Names may not exceed length of ${AVMConstants.ASSETNAMELEN}`,
      );
    }
    if (symbol.length > AVMConstants.SYMBOLMAXLEN) {
      /* istanbul ignore next */
      throw new Error(
        `Error - AVMAPI.buildCreateNFTAssetTx: Symbols may not exceed length of ${AVMConstants.SYMBOLMAXLEN}`,
      );
    }
    const avaxAssetID: Buffer = await this.getAVAXAssetID();
  ...
  issueTx = async (tx: string | Buffer | Tx): Promise<string> => {
    ...
    if (typeof tx === 'string') {
      ...
      /* istanbul ignore next */
      throw new Error(
        'Error - avm.issueTx: provided tx is not expected type of string, Buffer, or Tx',
      );
    }
    const params: any = {
      tx: Transaction.toString(),
    };
    return this.callMethod('avm.issueTx', params).then(
      (response: RequestResponseData) => response.data.result.txID,
    );
  };
  ...
  send = async (
    ...
    amount: number | BN,
    to: string,
    from: Array<string> | Array<Buffer>,
    changeAddr: string,
    memo: string | Buffer,
    ...
    return this.callMethod('avm.send', params).then(
      (response: RequestResponseData) => response.data.result,
    );
  };
  ...
  sendMultiple = async (
    ...
    let asset: string;
    let amnt: BN;
    const sOutputs: Array<{ assetID: string; amount: string; to: string }> = [];
    ...
    return this.callMethod('avm.sendMultiple', params).then(
      (response: RequestResponseData) => response.data.result,
    );
  };
  ...
  buildGenesis = async (genesisData: object): Promise<string> => {
    ...
    return this.callMethod('avm.buildGenesis', params).then(
      (response: RequestResponseData) => {
  ...
  constructor(
    ...
    this.blockchainID = blockchainID;
    const blockchain = this.getBlockchain(blockchainID);
    if (!blockchain) throw new Error('Blockchain ID not recognized');
    const alias = blockchain?.alias;
    this.keychain = new KeyChain(
      this.core.getHRP(),
      alias || blockchain?.blockchainID,
    );
  }
}
