...
import {
  AmountOutput, SelectOutputClass, TransferableOutput, NFTTransferOutput, NFTMintOutput, SECPMintOutput, SECPTransferOutput,
} from './outputs';
import { AVMConstants } from './constants';
import { UnsignedTx } from './tx';
import { SECPTransferInput, TransferableInput } from './inputs';
import {
  NFTTransferOperation, TransferableOperation, NFTMintOperation, SECPMintOperation,
} from './ops';
...
export class UTXOSet extends StandardUTXOSet<UTXO> {
  ...
  buildCreateAssetTx = (
    ...
    const CAtx:CreateAssetTx = new CreateAssetTx(networkid, blockchainid, outs, ins, memo, name, symbol, denomination, initialState);
  ...
  buildSECPMintTx = (
    ...
    const mintOp:SECPMintOperation = new SECPMintOperation(mintOwner, transferOwner);

    const utxo:UTXO = this.getUTXO(mintUTXOID);
    ...
    const out:SECPMintOutput = utxo.getOutput() as SECPMintOutput;
    ...
    const transferableOperation:TransferableOperation = new TransferableOperation(utxo.getAssetID(), [mintUTXOID], mintOp);
    ops.push(transferableOperation);

    const operationTx:OperationTx = new OperationTx(networkid, blockchainid, outs, ins, memo, ops);
  ...
  */
  buildCreateNFTMintTx = (
    ...
    const nftMintOperation: NFTMintOperation = new NFTMintOperation(groupID, payload, owners);
    ...
    for (let i:number = 0; i < utxoids.length; i++) {
      ...
      const transferableOperation:TransferableOperation = new TransferableOperation(utxo.getAssetID(), utxoids, nftMintOperation);
      ops.push(transferableOperation);
    }
