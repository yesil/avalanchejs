...
import { TransferableOutput, AmountOutput, ParseableOutput } from './outputs';
...
export class AddDelegatorTx extends WeightedValidatorTx {
  ...
  toBuffer():Buffer {
    ...
    const barr:Array<Buffer> = [super.toBuffer(), numouts];
  ...
  clone():this {
    const newbase:AddDelegatorTx = new AddDelegatorTx();
...
export class AddValidatorTx extends AddDelegatorTx {
  ...
  deserialize(fields:object, encoding:SerializedEncoding = 'hex') {
    super.deserialize(fields, encoding);
    const dbuff:Buffer = serializer.decoder(fields.delegationFee, encoding, 'decimalString', 'Buffer', 4);
  ...
  getDelegationFeeBuffer():Buffer {
    const dBuff:Buffer = Buffer.alloc(4);
    const buffnum:number = parseFloat(this.delegationFee.toFixed(4)) * AddValidatorTx.delegatorMultiplier;
  ...
  fromBuffer(bytes:Buffer, offset:number = 0):number {
    offset = super.fromBuffer(bytes, offset);
    const dbuff:Buffer = bintools.copyFrom(bytes, offset, offset + 4);
    offset += 4;
    this.delegationFee = dbuff.readUInt32BE(0) / AddValidatorTx.delegatorMultiplier;
    return offset;
  }
  ...
  toBuffer():Buffer {
    const superBuff:Buffer = super.toBuffer();
    const feeBuff:Buffer = this.getDelegationFeeBuffer();
